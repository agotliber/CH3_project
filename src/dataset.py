...
class Dataset():
    ...
    def current_position(self):
        return self._index_in_epoch * Conf.batch_size
...
def read_data_sets(filename_sequence,
    ...
    print(
        f"train_portion_subjects is {train_portion_subjects} , train_portion_probes is {train_portion_probes}, validation_portion_subjects is {validation_portion_subjects} and validation_portion_probes is {validation_portion_probes}")

    data_sequence_full = pd.read_csv(directory + filename_sequence)
    if data_sequence_full.columns[0] == 'Unnamed: 0':
        print("data_sequence_full.columns[0] == 'Unnamed: 0'")
        data_sequence_full = data_sequence_full.drop(columns=['Unnamed: 0'])
        if data_sequence_full.columns[0] != "Probe":
            print("error: Probe is not the first or second column of distances file")

    if isinstance(filename_expression, pd.DataFrame):
        data_expression_full = filename_expression
    else:
        data_expression_full = pd.read_csv(directory + filename_expression)
    Conf.exp_n_genes = data_expression_full.shape[1] - 1

    data_dist_full = pd.read_csv(directory + filename_dist)
    Conf.dist_n_genes = data_dist_full.shape[1] - 1

    if data_dist_full.columns[0] == 'Unnamed: 0':
        data_dist_full = data_dist_full.drop(columns=['Unnamed: 0'])
        if data_dist_full.columns[0] != "Probe":
            print("error: Probe is not the first or second column of distances file")
    ...
    if is_prediction:
        ...
        if data_labels.columns[0] == 'Unnamed: 0':
            data_labels = data_labels.drop(columns=['Unnamed: 0'])

    print(f"data_sequence_full-    probes x samples  {data_sequence_full.shape}")
    print(f"data_expression_full-  samples x genes   {data_expression_full.shape}")
    print(f"data_dist_full-        probes x genes    {data_dist_full.shape}")
    print(f"data_labels-           probes x samples  {data_labels.shape}")

    def replace_lowest_with_zeros(row):
        threshold = np.percentile(row, Conf.exp_th)  # Calculate the 20th percentile
        return np.where(row < threshold, 0, row)

    print(f"Conf.exp_th  {Conf.exp_th}")

    if Conf.exp_th > 0:
        df_with_zeros = data_expression_full.iloc[:, 1:].apply(replace_lowest_with_zeros, axis=0)
        data_expression_full.iloc[:, 1:] = df_with_zeros
    ...
    def dist_window(x):
        if x == 0:
            ...
            return 1 * np.sign(
                x)  # acts as an amplifier for the gene expr. data - if the gene is within the above window - tf.multiply gets the +/- value of the gene exp (depends if it's before or after CpG). Otherwise - gets 0.
            # return x
        else:
            return 0
    ...
    # scaling:
    data_expression_full = preprocessing.minmax_scale(data_expression_full[data_expression_full.columns[1:]], (10, 100),
                                                      axis=0)  # this is to give distances more of an effect (even if there's 0 gene expression (will be 0.5 in practice) then if the distance is not 0 will get some value
    ...
    if use_dist_window:
        data_sequence_full, data_labels, data_dist_full = drop_probes_with_no_pos_dist_in_window(data_sequence_full,
                                                                                                 data_labels,
                                                                                                 data_dist_full)

    # The other two are marked out here because the sacling above already returned them in an np.array format
    data_sequence_full = data_sequence_full.values  # in the future replace .as_matrix with .values as it will be deprecated in the future
    # data_expression_full = data_expression_full.values
    data_dist_full = data_dist_full.values
    if not is_prediction:
...
#             shuffled_idx = list(range(num_subjects))
#             np.random.shuffle(shuffled_idx)
#             print("splitting subjects into train/test_ch3_e_blind/val")
#             # subjects idx train/test_ch3_e_blind/val
#             train_subjects_idx = random.sample(shuffled_idx, int(round(num_subjects * train_portion_subjects)))
# #             np.random.shuffle(train_subjects_idx)
#             print(train_subjects_idx[:50])
#             print("\n")
#             test_subjects_idx = [i for i in shuffled_idx if i not in train_subjects_idx]
# #             np.random.shuffle(test_subjects_idx)
#             print(test_subjects_idx[:50])
#             validation_subjects_size = int(round(len(train_subjects_idx) * validation_portion_subjects))
#             validation_subjects_idx = train_subjects_idx[0:validation_subjects_size]
#             train_subjects_idx = train_subjects_idx[validation_subjects_size:]
            
            print("splitting subjects into train/test_ch3_e_blind/val")
            perm_num_subjects = np.arange(num_subjects)
            np.random.shuffle(perm_num_subjects)
       
            subj_train_size = int(round(num_subjects * train_portion_subjects))
            print(f"subjects train : 0 - {subj_train_size-1}")
            train_subjects_idx = perm_num_subjects[:subj_train_size]
            print(f"subjects test : {subj_train_size} - {len(perm_num_subjects)-1}")
            test_subjects_idx = perm_num_subjects[subj_train_size:]
            
            validation_subjects_size = int(round(len(train_subjects_idx) * validation_portion_subjects))
            print(f"subjects validation_subjects_size : {validation_subjects_size}")
            
            print(f"subjects val : 0 - {validation_subjects_size-1}")
            validation_subjects_idx = train_subjects_idx[0:validation_subjects_size]
            print(f"subjects train : {validation_subjects_size} - {len(train_subjects_idx)-1}")
            train_subjects_idx = train_subjects_idx[validation_subjects_size:]
            ...
            np.random.shuffle(perm_num_probes)
            # train_probes_idx = random.run_example(range(num_probes), int(round(num_probes * train_portion)))
            probes_train_size = int(round(num_probes * train_portion_probes))
            print(f"probes train : 0 - {probes_train_size-1}")
            train_probes_idx = perm_num_probes[:probes_train_size]
            print(f"probes test : {probes_train_size} - {len(perm_num_probes)-1}")
            test_probes_idx = perm_num_probes[probes_train_size:]
            
            validation_probes_size = int(round(len(train_probes_idx) * validation_portion_probes))
            print(f"probes val : 0 - {validation_subjects_size-1}")
            validation_probes_idx = train_probes_idx[0:validation_probes_size]
            print(f"probes train : {validation_probes_size} - {len(train_probes_idx)-1}")
            train_probes_idx = train_probes_idx[validation_probes_size:]
    ...
    train = Dataset(features_img, features_other, features_dist,
                    ...
                    is_prediction=is_prediction, expression_sample_names=expression_sample_names,
                    cpg_id_names=cpg_id_names)
    # fully blind val and test:
    validation_ch3_e_blind = Dataset(features_img, features_other, features_dist,
                                     validation_probes_idx, validation_subjects_idx, labels=data_labels,
                                     labels_raw=data_labels_raw,
                                     is_prediction=is_prediction, expression_sample_names=expression_sample_names,
                                     cpg_id_names=cpg_id_names)
    ...
    validation_ch3_blind = Dataset(features_img, features_other, features_dist,
                                   validation_probes_idx, train_subjects_idx, labels=data_labels,
                                   labels_raw=data_labels_raw, is_prediction=is_prediction,
    ...
    test_ch3_blind = Dataset(features_img, features_other, features_dist,
                             test_probes_idx, train_subjects_idx, labels=data_labels, labels_raw=data_labels_raw,
                             is_prediction=is_prediction,
                             expression_sample_names=expression_sample_names, cpg_id_names=cpg_id_names)
    # blind e (ch3 = train idx, e = val/test idx)
    validation_e_blind = Dataset(features_img, features_other, features_dist,
                                 train_probes_idx, validation_subjects_idx, labels=data_labels,
                                 labels_raw=data_labels_raw,
                                 is_prediction=is_prediction, expression_sample_names=expression_sample_names,
                                 cpg_id_names=cpg_id_names)
